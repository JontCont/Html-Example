...
<%
	...
	Vector<QueryResult> nps = app_sm.queryResult("select * from " + tblnp + " where DATE_FORMAT(np_emitdate,'%Y') like '%" + qyear + "%' AND np_lang='" + lang + "' AND np_code='" + page_code + "'" + result_str + "ORDER BY np_showseq ASC , np_emitdate DESC , np_createdate DESC");
			...
			<div class="right">
				...
				<div class="NewsListArea">
					...
					<div class="NewsList">
						...
						<div class="NL_tit">
							<a href="../support/volunteer_in.jsp?no=<%=np.getString("np_id")%>" title="<%=AppConfig.getProperty("openLink."+lang) %><%=np.getString("np_no").trim().equals("") ? "" : "(" + np.getString("np_no") + ")"%><%=np.getString("np_title")%>">
