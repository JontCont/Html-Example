...
<%
	...
	//若查無資料,跳回列表頁
	Vector<TableRecord> cps = app_sm.selectAll(tblcp, "cp_id=? AND cp_lang=? AND (? BETWEEN DATE_FORMAT(cp_emitdate,'%Y/%m/%d') and DATE_FORMAT(cp_restdate,'%Y/%m/%d'))", new Object[] { no, lang, app_today }, "cp_showseq ASC , cp_emitdate DESC , cp_createdate DESC");
	if (cps.size() == 0) {
		if (lang.equals("tw")) {
			out.write("<script>alert('抱歉，此功能正在維護中，請稍候在試！'); location='../service/disease.jsp'; </script>");
		} else {
			out.write("<script>alert('Sorry, This feature is not available right now. Please try again later!'); location='../service/disease.jsp'; </script>");
		}
		return;
	}
%>
