...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
		<%if(lang.equals("tw")){ %>
        <title>試吃申請成功通知信</title>
		<%} %>
		<%if(lang.equals("cn")){ %>
		<title>试吃申请成功通知信</title>
		<%} %>
		<%if(lang.equals("en")){ %>
		<title>Register notification letter</title>
		<%} %>
		<%if(lang.equals("jp")){ %>
		<title>試飲アプリケーションの成功の通知</title>
		<%} %>
</head>
...
<body>
        	...
        	<table width="571" border="0" cellpadding="0" cellspacing="0" style="font-size:12px;  line-height:20px; margin-top:10px; margin-bottom:10px;">
              ...
              <tr>
                <td colspan="2" align="center" style="font-size:15px; color: #000; letter-spacing:1.3px; line-height:24px; padding-left:6px; padding-right:6px; padding-bottom:10px; padding-top:10px; border-bottom: solid 1px #ccc;">
                	<%if(lang.equals("tw")){ %>
			        <strong>- 試吃申請成功通知信 -</strong>
					<%} %>
					<%if(lang.equals("cn")){ %>
					<strong>- 试吃申请成功通知信 -</strong>
					<%} %>
					<%if(lang.equals("en")){ %>
					<strong>- Notification of successful tasting application -</strong>
					<%} %>
					<%if(lang.equals("jp")){ %>
					<strong>- 試飲アプリケーションの成功の通知 -</strong>
					<%} %>
                </td>
              </tr>
              <tr>
	...
	<%if(lang.equals("tw")){ %>
		親愛的 <%=mp.getString("mp_name") %> 您好，我們已經收到您的試吃申請<br/>
		我們將會有專員盡速和您連絡! <br/>
	<%} %>
<%if(lang.equals("cn")){ %>
亲爱的 <%=mp.getString("mp_name") %> 您好，我们已经收到您的试吃申请<br/>
我们将会有专员尽速和您连络! <br/>
<%} %>
<%if(lang.equals("en")){ %>
Hello <%=mp.getString("mp_name") %> Hello, we have received your application for a taste<br/>
We will contact you as soon as possible! <br/>
<%} %>
<%if(lang.equals("jp")){ %>
親愛なるの <%=mp.getString("mp_name") %>こんにちは、私たちは味の申請を受け取りました<BR/>
スペシャリストができるだけ早くご連絡いたします！<br/>
<%} %>
                </td>
              </tr>              
            </table>
